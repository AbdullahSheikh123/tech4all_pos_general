...
def get_item_with_variant_and_rate_multi_group():
    ...
    for item_group in item_groups:
        try:
            # SQL query to fetch items with variants
            item_docs_list = frappe.db.sql("""
                SELECT name, item_code, item_name, description, image
                FROM `tabItem`
                WHERE item_group = %s AND has_variants = 1
            """, item_group, as_dict=True)

            items_list_without_variant = frappe.db.sql("""
            SELECT *
            FROM `tabItem`
            WHERE item_group = %s AND (variant_of IS NULL OR variant_of = '') AND disabled = 0 AND is_sales_item = 1 AND has_variants=0
        """, (item_group,), as_dict=True)
            ...
            for item in items_list_without_variant:
                item_code = item.get("item_code")
                
                # SQL query to fetch item price list
                item_price_list = frappe.db.sql("""
                ...
                for item_price in item_price_list:
                    # SQL query to fetch price list data
                    price_list_data = frappe.db.sql("""
                        SELECT price_list_name, selling, order_type
                        FROM `tabPrice List`
                        WHERE selling = 1 AND name = %s
                    """, item_price.price_list, as_dict=True)

                    item_price['order_type'] = price_list_data[0].get("order_type") if price_list_data else None

                # SQL queries for child table data (KDS and order type)
                kds_data = frappe.db.sql("""
                    SELECT * 
                    FROM `tabKDS Station Child`
                    WHERE parent = %s
                """, item.get("name"), as_dict=True)

                order_type_data = frappe.db.sql("""
                    SELECT * 
                    FROM `tabItem Order Type`
                    WHERE parent = %s
                """, item.get("name"), as_dict=True)

              # Get add-ons if custom_has_add_on is enabled
                addons = []
                if item.custom_has_add_on:
                    addon_names = frappe.get_all(
                        "Item Add-Ons",
                    ...
                    for addon in addon_names:
                        addon_doc = frappe.get_doc("Item Add-Ons", addon.name)
                        addons.append(addon_doc.as_dict())


                # Prepare final item dict
                variant_dict_without_variant = {
                    ...
                    "kds_": kds_data,  # Collect all kds_ values
                    "order_type": order_type_data,  # Collect all order_type values
                    "addons": addons
                }
            ...
            for item in item_docs_list:
                ...
                # SQL queries for child table data for the template
                kds_data = frappe.db.sql("""
                    SELECT * 
                    FROM `tabKDS Station Child`
                    WHERE parent = %s
                ...
                item_template_data["kds_"] = kds_data
                item_template_data["order_type"] = order_type_data

                # SQL query to fetch variants of the current item
                ...
                for variant in item_variants:
                    ...
                    # SQL query to fetch item price list for the variant
                    item_price_list = frappe.db.sql("""
                        SELECT price_list, price_list_rate
                        FROM `tabItem Price`
                        WHERE item_code = %s
                    ...
                    for item_price in item_price_list:
                        # SQL query to fetch price list data for the variant
                        price_list_data = frappe.db.sql("""
                            SELECT price_list_name, selling, order_type
                            FROM `tabPrice List`
                            WHERE selling = 1 AND name = %s
                    ...
                    # SQL queries for child table data for the variant
                    kds_data = frappe.db.sql("""
                        SELECT * 
                        FROM `tabKDS Station Child`
                        WHERE parent = %s
                    """, variant.get("name"), as_dict=True)
                    ...
                    variant_dict = {
                        ...
                        "item_group": variant.get("item_group"),
                    ...
                    item_template_data["variants"].append(variant_dict)

                item_detail.append({"template": item_template_data})
