frappe.provide('tech4all');
frappe.provide("tech4all.utils");

tech4all.utils.GeneralActivities = class GeneralActivities {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		// open activities
		frappe.call({
			method: "tech4all_pos_general.utils.get_open_activities",
			args: {
			...
			callback: (r) => {
				if (!r.exc) {
					var activities_html = frappe.render_template('general_activities', {
...
tech4all.utils.GeneralNotes = class GeneralNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		let notes_html = frappe.render_template('general_notes', {notes: notes});
...
frappe.ui.form.on("Customer", {
	refresh: function(frm) {
		const general_notes = new tech4all.utils.GeneralNotes({
			frm: frm,
			notes_wrapper: $(frm.fields_dict.notes_html.wrapper),
		});
		general_notes.refresh();

		const general_activities = new tech4all.utils.GeneralActivities({
			frm: frm,
