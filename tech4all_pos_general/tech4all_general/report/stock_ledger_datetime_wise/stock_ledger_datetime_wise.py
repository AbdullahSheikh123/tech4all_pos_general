...
def get_stock_ledger_entries(filters, items):
	...
	query = (
		...
		.where(
			(sle.docstatus < 2)
			& (sle.is_cancelled == 0)
			& (CombineDatetime(sle.posting_date, sle.posting_time)[filters.from_date : filters.to_date])
		)
...
def get_opening_balance(filters, columns, sl_entries):
	...
	last_entry = get_previous_sle(
		{
			...
			"posting_date": filters.from_date.split(" ")[0],
		}
	)
	...
	for sle in list(sl_entries):
		if (
			...
			and sle.posting_date == filters.from_date.split(" ")[0]
