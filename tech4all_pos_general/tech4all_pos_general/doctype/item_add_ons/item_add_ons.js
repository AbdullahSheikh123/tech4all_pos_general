...
frappe.ui.form.on('Item Add-Ons', {
	refresh: function(frm) {
		...
		frm.set_query("dependent_item_attribute", function() {
			if(frm.doc.item != ""){
				return {
					query: "tech4all_pos_general.tech4all_pos_general.doctype.item_add_ons.item_add_ons.item_attribute_query",
					filters: {
						"item": frm.doc.item
					}
				}
	...
	dependent_item_attribute:function(frm){
		if(frm.doc.dependent_item_attribute){
			frappe.call({
				method: 'tech4all_pos_general.tech4all_pos_general.doctype.item_add_ons.item_add_ons.get_attr_values',
				args:{
					item:frm.doc.item,
...
frappe.ui.form.on('Item Add Ons Child', {
	item:function(frm,cdt,cdn){
		...
		if (cdoc.item){
			frappe.call({
				method: "tech4all_pos_general.tech4all_pos_general.doctype.item_add_ons.item_add_ons.get_price_list_rate",
				args: {
					item_code: cdoc.item,
					uom: cdoc.uom
				},
