...
def get_data(filters):
    ...
    cash_flow_query = f"""
        SELECT jea.account,a.account_name, SUM(jea.debit) AS debit
        ...
        GROUP BY jea.account,a.account_name
    ...
    data.append({"group": "<b>SALES BY PAYMENT TYPES</b>",
                "des": "", "amount": ""})
    r_sale = abs(flt(sales_analysis[0]['r_sale']))
    for i in payment_row:
        if i['payment_type'] == 'Bank':
            data.append(
                # {"group": "", "des": i['payment_type'], "amount": i['adjusted_total']})
                {"group": "", "des": i['payment_type'], "amount": flt(i['adjusted_total']) })
    ...
    if sales_analysis:
        ...
        for i in payment_row:
            if i['payment_type'] != 'Bank': 
                data.append(
                    # {"group": "", "des": i['payment_type'], "amount": i['adjusted_total']})
                    {"group": "", "des": i['payment_type'], "amount": flt(i['adjusted_total'])-total_purchase +abs(flt(sales_analysis[0]['r_sale']))  })
    total_payment = 0
    for c in cash_flow:
        data.append({"group": "", "des": c['account_name'], "amount": c['debit']})
    ...
    if sales_analysis:
        for i in payment_row:
            if i['payment_type'] != 'Bank': 
                data.append(
                    # {"group": "", "des": i['payment_type'], "amount": i['adjusted_total']})
                    {"group": "", "des": "<b>Cash Balance</b>", "amount": flt(i['adjusted_total'])-total_purchase-total_payment+abs(flt(sales_analysis[0]['r_sale']))  })
    for i in payment_row:
            if i['payment_type'] == 'Bank': 
                data.append(
                    # {"group": "", "des": i['payment_type'], "amount": i['adjusted_total']})
