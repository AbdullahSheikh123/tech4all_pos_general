# Copyright (c) 2022, tech4allERO and contributors
...
def get_data(filters):
    ...
    for gr in get_item_group:
        ...
        for bol in check_bol:
            ...
            actual_usage=flt(bol.opening_qty)+flt(transfer_in_qty)-flt(transfer_out_qty)-flt(purchase_return_qty)
            variance=flt(bol.closing_qty)
            variance_cost=flt(flt(bol.closing_qty)) * flt(bol.last_purchase_rate)
            tot_variance += flt(variance_cost)
            if physical_stock != 0:
                row1 = []
                row1 = ['',bol.item_code,bol.item_name,bol.warehouse,bol.last_purchase_rate,bol.stock_uom,bol.opening_qty,purchase_qty,transfer_in_qty,transfer_out_qty,purchase_return_qty,adjustment_qty,theoretical_usage_qty,theoretical_usage_amount,actual_usage,bol.closing_qty,'Not Entered',variance,variance_cost]
                data.append(row1)
